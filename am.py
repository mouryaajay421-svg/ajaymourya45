...
if opt=="Home":
    ...
    col4.metric("⭐ Avg Rating",round(df["Google review rating"].mean(),2))
...
""")
    ...
    with t5:
        st.title("FIRST DATASET")
        df.drop(columns=["Unnamed: 0","Weekly Off"],inplace=True)
    ...
    "Number of google review in lakhs",
     ...
     fig = px.bar(
    top_places,
    x="Number of google review in lakhs",
    y="Name",
    orientation="h",
    color="Number of google review in lakhs",
    color_continuous_scale="Viridis",
    text="Number of google review in lakhs",
   ...
   yaxis=dict(categoryorder="total ascending")
    ...
    xaxis_tickangle=-45
     ...
     fig0=px.sunburst(df,path=["Zone","State","Name"],values="Google review rating",title="🌍 Tourism Hierarchy: Zone → State → Tourist Place")
...
# Top 10 recovered monuments
    ...
    with t4:
        dslr_sig = (df.groupby(["Significance", "DSLR Allowed"]).size().reset_index(name="Count"))
        fig7 = px.bar(dslr_sig,x="Significance",y="Count",color="DSLR Allowed",barmode="group",text="Count",color_discrete_map={    "Yes": "#2E8B57",    "No": "#E74C3C"},
       title="📷 DSLR Permission by Significance")
        ...
        fig8= px.box(df, x="State", y="Entrance Fee in INR", color="State", title="💰 Entrance Fee Distribution by State")
        fig8.update_layout(template="plotly_white",title_x=0.5,height=600,showlegend=False,xaxis_tickangle=-45)
        st.plotly_chart(fig8, use_container_width=True)

        fig0=px.sunburst(df,path=["Significance","Type","City"],values="Google review rating",title="Places Significance And Type")
        st.plotly_chart(fig0)    
    ...
    for _, row in result.iterrows():
      ...
      with st.container(border=True):
        ...
        with col1:
            st.metric("⭐ Rating", row["Google review rating"])
            st.metric("📝 Reviews", f"{int(row['Number of google review in lakhs']):,}")

        with col2:
            st.metric("💰 Entrance Fee", f"₹ {row['Entrance Fee in INR']}")
            st.metric("📷 DSLR", row["DSLR Allowed"])
        ...
        with right:
            ...
            st.write(f"**🛫 Nearby Airport:** {row['Airport with 50km Radius']}")
    ...
    if st.button("✨ Generate Insights", use_container_width=True):
     ...
     st.info(f"⭐ Highest average rating: **{df.groupby('State')['Google review rating'].mean().idxmax()}**")
 
     st.info(f"💰 Highest average entrance fee: **{df.groupby('State')['Entrance Fee in INR'].mean().idxmax()}**")

     st.info(f"📷 DSLR Allowed at **{(df['DSLR Allowed']=='Yes').mean()*100:.1f}%** of tourist places.")
    ...
    with st.container(border=True):
     st.write(f"⭐ **{df.groupby('State')['Google review rating'].mean().idxmax()}** has the highest average tourist rating.")

    with st.container(border=True):
     st.write(f"📷 **{(df['DSLR Allowed'].eq('Yes').mean()*100):.1f}%** of tourist places allow DSLR photography.")

    with st.container(border=True):
     st.write(f"💰 **{df.groupby('State')['Entrance Fee in INR'].mean().idxmax()}** has the highest average entrance fee.")

    with st.container(border=True):
     st.write(f"🌳 **{df['Significance'].mode()[0]}** is the most common tourist significance.")

    with st.container(border=True):
     st.write(f"📅 **{df['Best Time to visit'].mode()[0]}** is the most recommended season for tourists.")
